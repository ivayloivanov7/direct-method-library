...
class DirectMethodMqttClient:
    ...
    def connect(self, timeout: float = 5.0) -> None:
        ...
        with self._connection_lock:
            ...
            try:
                logger.info(
                    f"Connecting to MQTT broker at {self._broker_host}:{self._broker_port}"
                )

                # Connect to broker
                result_code = self._client.connect(
                    host=self._broker_host, port=self._broker_port, keepalive=60
                )

                if result_code != mqtt.MQTT_ERR_SUCCESS:
                    raise ConnectionError(
                        f"Failed to initiate connection: {mqtt.error_string(result_code)}"
                    )
    ...
    def _on_connect(
        ...
        if rc == 0:
            ...
            logger.info(
                f"Connected to MQTT broker at {self._broker_host}:{self._broker_port}"
            )
        else:
            self._is_connected = False
            error_msg = (
                f"Connection failed with result code {rc}: {mqtt.connack_string(rc)}"
            )
    ...
    def _on_subscribe(
        self, client: mqtt.Client, userdata: object, mid: int, granted_qos: list
    ) -> None:
        """Callback for when the broker responds to a subscribe request."""
        logger.debug(f"Subscription {mid} granted with QoS: {granted_qos}")

    def _on_log(
        self, client: mqtt.Client, userdata: object, level: int, buf: str
    ) -> None:
